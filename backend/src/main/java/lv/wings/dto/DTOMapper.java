...
public class DTOMapper {
	...
	public static <T extends DTOObject> ArrayList<T> mapMany(Class<T> responseType, Object[] targets,
			String[] ignoreFields) throws Exception {
		return mapMany(responseType, targets, ignoreFields, new String[0]);
	}

	public static <T extends DTOObject> ArrayList<T> mapMany(Class<T> responseType, Object[] targets,
			String[] ignoreFields, String[] forceFields) throws Exception {
	...
	public static <T extends DTOObject> T map(Class<T> responseType, Object target, String[] ignoreFields)
			throws Exception {
		return map(responseType, target, ignoreFields, new String[0]);
	}

	public static <T extends DTOObject> T map(Class<T> responseType, Object target, String[] ignoreFields,
			String[] forceFields) throws Exception {
		...
		fieldLoop: for (Field resourceField : resourceFields) {
			...
			if (fieldClass.toString().equals(methodType.toString())) {
				...
				if (fieldType instanceof ParameterizedType) {
					...
					// creating a collection type with its default non generic constructor (TODO:
					// find a way to create a generic object)
					...
					if (toArrayMethod != null) {
						...
						for (Object elem : allContentsAsArray) {
							addMethod.invoke(newObj,
									map((Class<T>) newFieldCollectionContentType, elem,
											flatterIgnoreFields(fieldName, ignoreFields),
											flatterIgnoreFields(fieldName, forceFields)));
						}
					}
					...
					resourceField.set(response, map((Class<T>) newFieldClass, executionResult,
							flatterIgnoreFields(fieldName, ignoreFields), flatterIgnoreFields(fieldName, forceFields)));
				}
			}
		}
	...
	private static ArrayList<Method> getAllMethods(Class<?> classDef) {
		...
		} while ((inheritParent = inheritParent.getSuperclass()) != null
				&& inheritParent != inheritParent.getSuperclass());
