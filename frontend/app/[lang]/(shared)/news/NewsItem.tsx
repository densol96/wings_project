    ...
    <div className="flex flex-col shadow-md rounded-xl overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 max-w-sm w-full mx-auto">
      <div className="h-auto overflow-hidden">
        <div className="h-44 overflow-hidden relative">
          <Image
            className="object-cover object-top transform transition duration-300 hover:scale-110"
            fill
            src={item.image?.src || `https://www.allianceplast.com/wp-content/uploads/no-image.png`}
            alt={item.image?.alt || dict.alt}
          />
        </div>
      </div>
      <div className="bg-white py-4 px-6">
        ...
        <p className="text-sm text-gray-500 break-words mt-2">
          {description?.length > 20 ? `${description.slice(0, 125)}...` : description}
        </p>
