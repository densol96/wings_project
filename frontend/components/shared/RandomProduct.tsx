...
const RandomProduct = ({ product, lang }: Props) => {
  return (
    <div key={product.id} className="py-20 px-10">
      <div
        key={product.id}
        className="shadow-custom-med lg:h-[350px] h-auto flex lg:flex-row flex-col items-center 2xl:gap-32 lg:gap-10 gap-2 rounded-b-none lg:rounded-3xl border-primary-bright bg-gradient-to-t from-light-nav to-white shadow-custom-def overflow-hidden lg:overflow-visible rounded-t-3xl"
      >
        <AnimatedProductImage
          images={product.imageDtos}
          lang={lang}
          className={cn(
            "h-[400px] w-full lg:h-full lg:w-[35%] lg:ml-28 transform lg:scale-125 scale-100 bg-gray-50 lg:shadow-custom-med shadow-none rounded-b-none  lg:rounded-3xl",
