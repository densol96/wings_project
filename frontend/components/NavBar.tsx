...
const NavBar: React.FC<Props> = ({ className, children, isExpanded, headerDictionary }) => {
  ...
  return (
    <div
      className={`${
        isExpanded ? "opacity-100 grow" : `invisible`
      } shrink-0 z-10 p-10 bg-cover flex justify-center items-center  bg-[url('/assets/knitting_img.jpg')] h-full bg-no-repeat bg-center rounded-xl overflow-hidden lg:visible lg:grow-0`}
    >
      <ul
        className={`${
      ...
      >
        {Object.values(headerDictionary).map((item) => (
          <li key={item.title}>
            <Link className={path.startsWith(item.href) ? active : def} href={item.href}>
              {item.title}
            </Link>
          </li>
        ))}
